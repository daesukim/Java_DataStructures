public class NodeBST<E extends Comparable<E>> extends BST<E> {
    ...
    public BST<E> insertAndBalance (E elem) {
        ...
        if (!result.isBalanced()){
            result = TreeTraversals.balance(result);
        }
        return result;
    }
    ...
    public BST<E> delete(E elem) throws EmptyTreeExc {
        ...
        if (elem.equals(data)){
            if (!left.isEmpty()){
                E max = left.findMax();
                result = new NodeBST<>(max, left.deleteMax(), right);
            }
            else{
                result = right;
            }
            ...
            result = new NodeBST<>(data, left.delete(elem), right);
        }
        else if (elem.compareTo(data) > 0) {
            result = new NodeBST<>(data, left, right.delete(elem));
        }
        return result;
    }
    ...
    public E findMin() throws EmptyTreeExc {
        if (!left.isEmpty()){
            return left.findMin();
        }
        else{
    ...
    public E findMax() throws EmptyTreeExc {
        if (!right.isEmpty()){
    ...
    public BST<E> deleteMin() throws EmptyTreeExc {
        if (!left.isEmpty()){
    ...
    public BST<E> deleteMax() throws EmptyTreeExc {
        if (!right.isEmpty()){
