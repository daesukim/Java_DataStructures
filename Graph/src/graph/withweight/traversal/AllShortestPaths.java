...
public class AllShortestPaths extends WeightedIterativeGraphTraversal {
    ...
    // inserted to look at previousNodes
    ...
    public void relaxEdge(Edge edge) {
        ...
        if (!visited.contains(destination)) {
            Weight currentNodeWeight = heap.getWeight(source);
            Weight newWeight = weights.get(edge).add(currentNodeWeight);
    ...
    public Path getPath (String destination) {
        ...
        while (previousNodes.containsKey(current)) {
            Edge edge = new Edge(previousNodes.get(current), current);
            Weight weight = weights.get(edge);
