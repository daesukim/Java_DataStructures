...
public class Heap extends NodeCollection {
    ...
    public void moveDown(String n) {
        if (getMinChild(n).isEmpty()) return;
        else if (weights.get(n).compareTo(weights.get(getMinChild(n).get())) > 0){
    ...
    public void moveUp(String n) {
        if (getParent(n).isEmpty()) return;
        else if (weights.get(n).compareTo(weights.get(getParent(n).get())) < 0){
    ...
    public void insert(String n, Weight w) {
        ...
        indices.put(n, size);
        size++;
        if (w.value() < weights.get(getParent(n).get()).value()) moveUp(n);
    }
